...
#Run the GSA based on different levels 
#Only look at certain level of functionality, first indicate what rows to keep
...
#Then subset res_all to only keep those rows
...
for (i in 1:length(comp_list)) {
    TopTable <- as.data.frame(comp_list[i], col.names = "", row.names = 1) #create TopTable for analysis
    ...
    A <- consGSAplot(resList=res_Bars_BP, rankScore=25, Pcutoff=0.01, distinct='distinct', 
                     savePlot=FALSE, title='Biological Process - rankScore 50, p<0.01')
    B <- consGSAplot(resList=res_Bars_MF, rankScore=25, Pcutoff=0.01, distinct='distinct', 
                     savePlot=FALSE, title='Molecular Function - rankScore 50, p<0.01')
    C <- consGSAplot(resList=res_Bars_CC, rankScore=25, Pcutoff=0.01, distinct='distinct', 
                     savePlot=FALSE, title='Cellular Component - rankScore 50, p<0.01')
