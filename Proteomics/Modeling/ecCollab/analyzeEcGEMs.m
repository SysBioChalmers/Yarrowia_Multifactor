...
mean_fluxes_model1 = mean(full(allSol4Plot.ecJFYL07), 2);
mean_fluxes_model2 = mean(full(allSol4Plot.ecJFYL14), 2);

% Calculate fold changes (or differences in means)
fold_changes = mean_fluxes_model2 ./ mean_fluxes_model1;

% Perform two-sample t-test to compute p-values
[~, p_values] = ttest2(full(allSol4Plot.ecJFYL14), full(allSol4Plot.ecJFYL07), 'Dim', 2);
...
fold_changes = fold_changes.*(abs(mean_fluxes_model1) > fluxCutOff).*(abs(mean_fluxes_model1) > fluxCutOff);
